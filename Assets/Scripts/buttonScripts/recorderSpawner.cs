...
public class recorderSpawner : MonoBehaviour {
    ...
    {
        arrayOfButtons = new Button[GlobalVariables.recorderRange.Length];
        ...
        float i = 10f;
        int[] range = GlobalVariables.recorderRange;
    ...
    void spawnRecorder(int lane)
    {
       ...
       for (int i = 0; i < arrayOfButtons.Length; i ++)
        ...
        for (int i = 0; i < GlobalVariables.spaceFilled[lane+1].Length; i++)
        {
            ...
            {
                Instantiate(recorderPrefab, new Vector3(-6.0f - 2f * i, -9.8f + (lane+1) * 2f, 0f), transform.rotation);
